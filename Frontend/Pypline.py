...
window_sizes = np.arange(15, 31)
test_size = 31
ds_predict_31_day, today = Predictor.predict_sma_tickers(tickers, stocks_data, target_column, window_sizes, test_size)

# Строим график
# Переменный тикер, который вручную меняется
ticker = 'AMZN'
# На выбор ставим в ds_predict_X_day 3, 4, 7, 14, 31 день
df_future = ds_predict_14_day[ticker]['predict']
name_company = f'Тикер компании- {ticker}'
fig, ax = plt.subplots(figsize=(18, 8))
# Фон графика
ax.axvspan(df_future['date'].min(), df_future['date'].max(), facecolor='lightgray', alpha=0.5)
# Кривая для фактического таргета
plt.plot(df_future['date'], df_future['target'], label='Rate', color='blue', linewidth=3)
# Кривая для лин.модели
plt.plot(df_future['date'], df_future['predict'], label='Predict', color='red', linewidth=3)
# Добавляем вертикальную пунктирную линию (окончание обучающего периода)
train_end_date = pd.to_datetime(today)
ax.axvline(today, color='black', linestyle='--')
# Поворот меток оси x для более удобного чтения
plt.xticks(rotation=45)
plt.legend(loc='upper right')
plt.title(name_company, loc="center", size=18, weight='bold')
plot = {'fig': fig, 'ax': ax}